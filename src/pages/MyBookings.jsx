...
export default function MyBookings() {
    ...
    async function fetchBookings() {
      try {
        const response = await axios.get("http://localhost:4005/bookings"); // Replace with your backend API endpoint
              ...
              >
                ...
                <div className="mt-6 sm:col-span-7 sm:mt-0 md:row-end-1">
                  <h3 className="text-lg font-medium text-gray-900">
                    <a href={booking.href}>{booking.name}</a>
                  </h3>
                  <p className="mt-1 font-medium text-gray-900">
                    {booking.price}
                  </p>
                  <p className="mt-3 text-gray-500">{booking.description}</p>
                </div>
                <div className="sm:col-span-12 md:col-span-7">
                  <dl className="grid grid-cols-1 gap-y-8 border-b border-gray-200 py-8 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10">
                    <div>
                      ...
                      <dd className="mt-3 text-gray-500">
                        <span className="block">{booking.address[0]}</span>
                        <span className="block">{booking.address[1]}</span>
                        <span className="block">{booking.address[2]}</span>
                      </dd>
                    </div>
                    <div>
                      <dt className="font-medium text-gray-900">
                  ...
                  <p className="mt-6 font-medium text-gray-900 md:mt-10">
                    {booking.status} on{" "}
                    <time dateTime={booking.datetime}>{booking.date}</time>
                  </p>
                  <div className="mt-6">
                    <div className="overflow-hidden rounded-full bg-gray-200">
                      <div
                        ...
                        style={{
                          width: `calc((${booking.step} * 2 + 1) / 8 * 100%)`,
                        }}
                      />
                    </div>
